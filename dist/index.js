...
class NatsError extends Error {
    ...
    static errorForCode(code, chainedError) {
        const m = Messages$1.getMessage(code);
...
class DenoBuffer {
    ...
    _reslice(len) {
        assert$2(len <= this._buf.buffer.byteLength);
...
const natsHeadersToRecord = (headers) => {
    ...
    for await (const message of subscription) {
        try {
            ...
            const send = (response, options = {}) => {
                const responseHeaders = headers(options.code, options.codeDescription);
...
const createConnection = async ({ server, creds, token, subscribers = {}, options = {}, env = {}, }) => {
    const { log = () => { } } = options;
...
const postWhisperAudioBytes = async ({ audio, url, params, signal, }) => {
    ...
    formData.append("audio_file", new Blob([audio], { type: "audio/wav" }), "audio.wav");
...
const sendAsrNonStreamingResponse = async ({ signal, detail, headers, send, sendError, env }) => {
    const { body } = detail;
...
const asrTranscribeListener = async (props) => {
    try {
